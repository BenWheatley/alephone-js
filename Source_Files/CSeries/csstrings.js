...
class MacRomanUnicodeConverter {
	constructor() {
		if (!MacRomanUnicodeConverter.initialized) {
			for (let c = 0x80; c < 0xff; c++) {
				MacRomanUnicodeConverter.unicode_to_mac_roman.set( MacRomanUnicodeConverter.mac_roman_to_unicode_table[c], c );
			}
			MacRomanUnicodeConverter.initialized = true;
		}
	}
	...
	static unicode_to_mac_roman = new Map();
...
const macRomanUnicodeConverter = new MacRomanUnicodeConverter();

function mac_roman_to_unicode(/* char */ c) {
	return macRomanUnicodeConverter.ToUnicode(c);
}

function unicode_to_mac_roman(/* uint16 */ c) {
	return macRomanUnicodeConverter.ToMacRoman(c);
}

function unicode_to_utf8(c, output) {
	if (c < 0x80) {
		return output + String.fromCharCode(c);
	}
	else if (c < 0x800) {
		return output + String.fromCharCode(0xc0 | c >> 6) + 
						String.fromCharCode(0x80 | (c & 0x3f));
	}
	else {
		return output + String.fromCharCode(0xe0 | c >> 12) + 
						String.fromCharCode(0x80 | (c >> 6 & 0x3f)) + 
						String.fromCharCode(0x80 | (c & 0x3f));
	}
}

/* TODO: decide if I even need this any more. I might, I just don't know yet.
...
function mac_roman_to_utf8(/* Uint8Array */ input) {
	let p = 0;
	let output = "";
	while (p < input.length) {
		let uc = mac_roman_to_unicode(input[p++]);
		output = unicode_to_utf8(uc, output);
	}
	return output;
}
